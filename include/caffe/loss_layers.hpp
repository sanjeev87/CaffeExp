...
/**
 * Computes gradient of Dr.Lim paper
 * This can be used to train siamese networks.
 */
template <typename Dtype>
class MineContrastiveLossLayer : public LossLayer<Dtype> {
 public:
  explicit MineContrastiveLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
  ...
  virtual inline int ExactNumBottomBlobs() const { return 3; }
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_MINE_CONTRASTIVE_LOSS;
  }
  /**
   * Unlike most loss layers, in the MineContrastiveLossLayer we can backpropagate
   * to the first two inputs.
   */
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    return bottom_index != 2;
 ...
 protected:
  /// @copydoc MineContrastiveLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
  virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
