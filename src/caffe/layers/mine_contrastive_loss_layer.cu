...
void MineContrastiveLossLayer<Dtype>::Forward_gpu(
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    if (static_cast<int>(bottom[2]->cpu_data()[i])) {  // similar pairs
      ...
      temp = std::max(margin-sqrt(dist_sq_.cpu_data()[i]), Dtype(0.0));
      loss += temp * temp;
    }
  }
...
__global__ void CLLForward(const int count, const int channels,
  ...
  CUDA_KERNEL_LOOP(i, count) {
    ...
    if (static_cast<int>(y[n])) {  // similar pairs
      ...
      if ((margin-sqrt(dist_sq[n])) > 0.0) {
        bottom_diff[i] = alpha * (1 - margin/sqrt(dist_sq[n])) * diff[i];
      } else {
        bottom_diff[i] = 0;
      }
    }
...
template <typename Dtype>
void MineContrastiveLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
